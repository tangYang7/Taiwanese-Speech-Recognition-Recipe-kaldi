...
if [ $stage -le -2 ]; then
  ...
  # local/prepare_data.sh --train-dir $train_dir --test-dir $test_dir --train_txt $train_txt --test_txt $test_txt || exit 1;
...
if [ $stage -le 7 ]; then
  ...
  local/chain/run_tdnn.sh --gpuid $gpuid
fi

# getting results (see RESULTS file)
if [ $stage -le 8 ]; then
  echo "$0: extract the results"
  for test_set in $test_set; do
  echo "WER: $test_set"
